...
def run_fast_integration_tests():
    print("============================================================")
    print(" STEP 3 ACCELERATED INTEGRATION TEST SUITE")
    print("============================================================")
    ...
    # Admin Login
    res = client.post("/api/auth/login", json={"email": "admin@fast.edu", "password": "AdminPass123!"})
    ...
    # Student A Login
    res = client.post("/api/auth/login", json={"email": "student_a@fast.edu", "password": "StudentPass123!"})
    assert res.status_code == 200
    ...
    # Student B Login
    res = client.post("/api/auth/login", json={"email": "student_b@fast.edu", "password": "StudentPass123!"})
    assert res.status_code == 200
    ...
    res = client.get(f"/api/face/student/{str(student_a_id)}", headers=student_a_headers)
    assert res.status_code == 200
    st_data = res.json()
    ...
    assert res.status_code == 403, f"Expected 403 Forbidden on ownership violation, got {res.status_code}"
    print("7. Role Security (Student B attempting to register Student A -> 403 Forbidden): PASS")
    ...
    assert res.status_code == 400, f"Expected 400 Bad Request on duplicate face, got {res.status_code}"
    ...
    img_unk = np.ones((640, 640, 3), dtype=np.uint8) * 100
    cv2.ellipse(img_unk, (320, 280), (130, 170), 0, 0, 360, (50, 80, 200), -1)
    cv2.circle(img_unk, (265, 245), 18, (255, 255, 255), -1)
    cv2.circle(img_unk, (375, 245), 18, (255, 255, 255), -1)
    cv2.circle(img_unk, (265, 245), 8, (200, 0, 0), -1)
    cv2.circle(img_unk, (375, 245), 8, (200, 0, 0), -1)
    ...
    res = client.delete(f"/api/face/student/{str(student_a_id)}", headers=admin_headers)
    assert res.status_code == 200
    ...
    assert res.status_code == 200
    assert res.json()["student_id"] != str(student_a_id)  # Deactivated embedding ignored!
